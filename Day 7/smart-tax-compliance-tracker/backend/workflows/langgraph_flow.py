...
# from ..agents import document_monitor, deadline_tracker, regulation_retriever
...
# Define node functions
def document_monitor_node(state: dict):
    state["document_status"] = doc_agent.monitor_documents()
    return state

def deadline_tracker_node(state: dict):
    state["deadlines"] = deadline_agent.get_upcoming_deadlines()
    return state

def regulation_retriever_node(state: dict):
