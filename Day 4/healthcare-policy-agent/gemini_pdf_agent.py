...
def process_query(user_query):
    ...
    for msg in st.session_state.history[-6:]:  # Keep last 6 messages
...
def main():
    ...
    with st.sidebar:
        ...
        st.session_state.user_profile['age'] = st.slider(
            "Your Age", 18, 100, 30,
            help="Select your current age"
        )
        
        st.session_state.user_profile['family_type'] = st.radio(
            "Coverage Type", 
            ["Individual", "Family"],
            index=0,
            help="Select whether you need individual or family coverage"
        )
        
        if st.session_state.user_profile['family_type'] == "Family":
            st.session_state.user_profile['dependents'] = st.number_input(
                "Number of Dependents", 1, 10, 2,
                help="How many family members need coverage"
            )
        else:
        ...
        st.session_state.user_profile['requirements'] = st.multiselect(
            ...
            help="Select any special healthcare needs"
        )
    ...
    with col2:
        if st.button("📋 Get Recommendations", use_container_width=True):
            st.session_state.history.append({
                "role": "user", 
                "content": "Recommend suitable policies based on my profile"
            })
            st.rerun()
    ...
    with col3:
        if st.button("📄 View Policy Details", use_container_width=True):
            st.session_state.history.append({
                "role": "user", 
                "content": "List all available policies with their key features"
            })
            st.rerun()
